    ...
    : BaseActivityDto
    {
        public BaseActivityValidator(Func<T, TDto> selector)
        {
            RuleFor(x => selector(x).Title)
                ...
                .MaximumLength(100).WithMessage("Title must not exceed 100 charectors");
            ...
            RuleFor(x => selector(x).Venue).NotEmpty().WithMessage("Vanue is required");
            RuleFor(x => selector(x).Latitude)
                .NotEmpty().WithMessage("Latitude is required")
                .InclusiveBetween(-90, 90).WithMessage("Latitute is required");
            RuleFor(x => selector(x).Longitude)
                .NotEmpty().WithMessage("Longitude is required")
                .InclusiveBetween(-180, 180).WithMessage("Longitude is required");
